...
\author{\IEEEauthorblockN{Aridaman Singh Nandan}
\IEEEauthorblockA{\textit{Department of Computer Science and Engg.} \\
\textit{NIT-Jalandhar}\\
Punjab, India \\
aridaman1995@gmail.com}
\and
\IEEEauthorblockN{Geeta Sikka}
\IEEEauthorblockA{\textit{Department of Computer Science and Engg.} \\
\textit{NIT-Jalandhar}\\
Punjab, India \\
sikkag@nitj.ac.in}
...
\FOR{$i \gets 1$ to $world\_population$}                    
\STATE {$organism\gets create\ new\ Organism\ object$}
\STATE {$population\xleftarrow{\text{add}} organism$}
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic} 
\STATE {$i\gets size\ of\ food\ list$}
\FOR {$i\geq 0$}
\STATE {$food\_location\gets food[i]$}
\STATE {$dist\gets distance(food\_location,\ organism\_location)$}
\IF {$dist\leq dna[2]$}
\STATE {$health\gets health\ +\ 100$}
\STATE {$food\ list \xleftarrow{\text{remove}} food\_location$}
\ENDIF
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
\caption{Function to eat poison}
\begin{algorithmic} 
\STATE {$i\gets size\ of\ poison\ list$}
\FOR {$i\geq 0$}
\STATE {$poison\_location\gets poison[i]$}
\STATE {$dist\gets distance(poison\_location,\ organism\_location)$}
\IF {$dist\leq dna[3]$}
\STATE {$health\gets health\ -\ 100$}
\STATE {$poison\ list \xleftarrow{\text{remove}} poison\_location$}
\ENDIF
\ENDFOR
\end{algorithmic}
\end{algorithm}

In this scenario organisms can eat food(Algorithm 3) to live longer and have likelihood to reproduce. The system will evolve organisms with ability to find and eat food.
...
\begin{algorithmic} 
\IF {$random(1)\leq 0.005$}
\STATE {$child\_dna\gets Make\ a\ copy\ of\ dna$}
\STATE {$child\_dna\gets mutate(child\_dna,\ 0.01)$}
\STATE {$child\gets new\ Organism(location,\ child\_dna)$}
\STATE {$return\ \textbf{child}$}
\ELSE
...
\begin{algorithmic} 
\FOR {$i\gets 0\ to\ 3$}
\IF {$random(1)\leq mutation\_rate$}
\IF {$i\ <\ 2$}
\STATE {$genes[i]\gets genes[i]\ +\ random(-0.2, 0.2)$}
\ELSE
\STATE {$genes[i]\gets genes[i]\ +\ random(-10, 10)$}
\ENDIF
\ENDIF
\ENDFOR
\end{algorithmic}
...
\bibitem{enhanced-genetic-algorithm}
...
\bibitem{real-coded genetic algorithms}
...
\bibitem{population-genetic-algorithms}
